...
For tables that compare or rank ticker-based ideas, use this column order whenever the relevant fields are available: **Rank**, **Ticker**, **Current stock price**, **Strike**, **Expiration / DTE**, **Delta**, **IV %**, **Premium received**, **ROI %**, **Cash required**, **Breakeven**, **Downside buffer %**, **Contracts affordable**, **Estimated monthly income**, **Stock quality score**. When both expiration and DTE are available, combine them in the single **Expiration / DTE** column (for example, `2026-08-21 (30 DTE)`). Keep **Stock quality score** as the final column. Omit option-specific columns only when the tool did not return a value for that metric; do not move the mandatory current stock price or stock quality score columns.
