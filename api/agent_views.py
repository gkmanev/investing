...
For every ticker displayed from any tool response, always include a **Technicals** field or table column. Use `technical_score` (or `technicals`, `tv_technicals`, or `tvTechnicals` when available); if it is unavailable, display `N/A`.

For tables that compare or rank ticker-based ideas, use this column order whenever the relevant fields are available: **Rank**, **Ticker**, **Price**, **Strike**, **Expiration / DTE**, **Delta**, **IV %**, **Premium received**, **ROI %**, **Cash required**, **Breakeven**, **Downside buffer %**, **Contracts affordable**, **Estimated monthly income**, **RSI**, **Technicals**, **Stock quality score**. When both expiration and DTE are available, combine them in the single **Expiration / DTE** column (for example, `2026-08-21 (30 DTE)`). Keep **Stock quality score** as the final column, immediately after Technicals when it is available. Omit option-specific columns only when the tool did not return a value for that metric; do not move the mandatory price, RSI, Technicals, or stock quality score columns.
