...
class FetchScreenerResultsCommandTests(APITestCase):
    ...
    @patch("api.management.commands.fetch_screener_results.requests.post")
    def test_command_merges_multiple_filter_dicts(self, mock_post: MagicMock) -> None:
        screener = ScreenerType.objects.create(
            name="Energy Income", description="Multiple filter entries."
        )
        ScreenerFilter.objects.create(
            screener_type=screener,
            label="Energy sector filter",
            payload={"filter": {"asset_primary_sector": {"eq": "Energy"}}},
            display_order=1,
        )
        ScreenerFilter.objects.create(
            screener_type=screener,
            label="Dividend filter",
            payload={"filter": {"dividend_yield": {"gte": 0.03}}},
            display_order=2,
        )
        ScreenerFilter.objects.create(
            screener_type=screener,
            label="Quant rating filter",
            payload={
                "quant_rating": {
                    "in": [
                        "strong_buy",
        ...
        self.assertIn("filter", payload)
        self.assertEqual(
            payload["filter"]["asset_primary_sector"].get("eq"), "Energy"
        )
        self.assertEqual(
            payload["filter"]["dividend_yield"].get("gte"), 0.03
        )
        self.assertIn("quant_rating", payload)
        self.assertEqual(payload["quant_rating"]["in"], ["strong_buy", "buy"])
        self.assertEqual(payload["filter"]["marketcap_display"]["gte"], 2_000_000_000)
